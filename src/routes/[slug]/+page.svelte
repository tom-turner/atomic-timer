...
<main
    class="flex flex-col items-center justify-between w-full h-screen space-y-8 p-8"
>
    <div class="fixed inset-0 -z-50 bg-white">
      <img src={"https://source.unsplash.com/random/1920x1080?nature"} class=" -z-40 w-full h-screen opacity-80 object-cover" alt="random-image" />
    </div>
    ...
    <div>
        ...
        {#each timers as timer}
            ...
            <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                ...
                <div class="flex w-full justify-evenly space-x-2 sm:space-x-0 flex-row sm:flex-col">
                    {#if timer.isRunning}
                      <button on:click={() => stopTimer(timer.id, timer.durationMs)} class="bg-yellow-400 hover:bg-yellow-500 w-full text-white my-auto font-bold py-2 px-4 rounded">Stop</button>
                    {:else}
                      <button on:click={() => startTimer(timer.id, timer.durationMs)} class="bg-green-400 hover:bg-green-500 w-full text-white my-auto font-bold py-2 px-4 rounded">Start</button>
                    {/if}
                    <EditTimer {page} {timer} onSubmit={() => {}}/>
                    <button on:click={() => deleteTimer(timer.id)} class="bg-red-400 hover:bg-red-500 text-white my-auto w-full font-bold py-2 px-4 rounded">Delete</button>
                </div>
            </div>
        {/each}
      </div>
      <button on:click={handleShare} class="bg-sky-400 hover:bg-sky-500 mx-auto flex mt-4 text-white font-bold py-2 px-4 rounded">Share</button>
      ...
      <div class="text-center text-white font-semibold text-sm text-shadow">
          <a class="hover:underline text-sky-400" href="https://ttcreative.limited" target="_blank">TT Creative</a>
